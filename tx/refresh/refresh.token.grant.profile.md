...
details how a provider access_token server can issue client-constrained refresh tokens and how confidential clients (
...
When starting a transfer process, an `access_token` may be generated by the provider's data plane access token server
...
- `refresh_endpoint` - OPTIONAL: The endpoint address for the data plane access_token server that handles token refresh.
  ...
  "dspace:dataAddress": {
    ...
    "dspace:endpointProperties": [
      ...
      },
      {
        ...
        "dspace:value": "https://example.com/access_token/refresh"
      }
    ]
  }
}
...
The client sends refresh token requests to the provider's data plane access_token server endpoint using the OAuth
...

{
  ...
  "refresh_endpoint": "https://example.com/access_token/refresh",
  "expires_in": "300"  
}
```
...
The provider data plane access_token server can verify the refresh request using the signed client JWT following these
steps:
...
Provider data plane access_token servers SHOULD implement the additional recommendations outlined
...
If validated and authorized, the provider data plane access_token server issues an access token as described in
the [OAuth 2 specification](https://datatracker.ietf.org/doc/html/rfc6749#section-5.1). Note that the access_token
